...
$pattern = '(?m)^(PGADMIN_ALLOWLIST)\s*=.*$'
$replacement = "`$1=$cidr"

$newText = [System.Text.RegularExpressions.Regex]::Replace($text, $pattern, $replacement)
$matched = ($newText -ne $text)
...
if ($confirm) {
    Write-Info ("Updated: " + ($confirm.Matches.Value))
