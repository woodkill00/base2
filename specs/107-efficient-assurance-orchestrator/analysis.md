...
## Cycle 9 — Independent review corrections

Independent code/security, operations, and UX/data review found no critical issue but rejected the candidate for several high and medium assurance gaps. A synthetic or partial gate receipt could satisfy automatic execution because the ordered current manifest and every referenced artifact were not jointly revalidated; explicit release could also reuse that receipt contrary to its contract. Source could change during a running check before cache publication, sensitive-path matching was incomplete on case-sensitive names, private runs and cache entries lacked fixed retention ceilings, and failure summaries omitted bounded attempt diagnostics.

The benchmark also compared complete-gate artifact bytes against routine-fixture summaries rather than two representations of the same exact gate. The correction validates the complete-gate manifest and all artifacts, reserves receipt reuse for non-explicit release requests, rechecks source before and after each process, makes sensitive matching case-insensitive, prunes only strictly owned private entries to fixed limits, adds redacted failure detail, closes the published JSON Schema, implements real text/JSON modes, and compares full exact-gate logs with the compact receipt for that same gate. T101-T112 bind these corrections to adversarial tests, renewed independent review, and one final exact-commit gate.

## Cycle 10 — Adversarial review closure

The renewed reviewers found that artifact validation still treated a missing artifact as optional, sensitive mapping omitted token/login/permission filenames, multiple cache writes in one run could exceed a start-of-run ceiling, and the benchmark’s own replay and per-commit storage needed the same integrity and retention discipline. They also identified the final publication edge of source revalidation, incomplete human explanation output, terminal control injection in diagnostic text, and an error-path evidence-link mismatch.

The final correction requires complete typed evidence for every required gate check, extends conservative security markers, enforces cache limits before every publication, checks source after cache reads and immediately before cache and terminal result publication, validates all benchmark logs plus their underlying exact gate, bounds benchmark commit directories, renders reasons/selected/avoided/residual scope, escapes every terminal control through a JSON-string-safe text transform, and attaches interrupted evidence when an admitted run errors. T113-T122 cover the implementation, hostile regressions, and tenth static analysis cycle before renewed acceptance.
