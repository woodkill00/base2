...
## Phase 66 — Gate ownership and successful-shard cycle 89 corrections

- [ ] B505 Acquire one nonblocking repository-wide complete-gate lock before source admission, check execution, or shared release-evidence mutation, with restrictive mode and automatic process/exception release. (FR-002, FR-005, FR-007, FR-008, FR-060, FR-063, FR-065, FR-068, FR-070)
- [ ] B506 Return a distinct busy exit and integrity-bound run-unique busy receipt without starting a check, reading source identity, or touching the active gate's evidence. (FR-002, FR-005, FR-007, FR-008, FR-031, FR-060, FR-063, FR-065, FR-068, FR-070)
- [ ] B507 Prove exact owner/contender rejection, restrictive lock mode, zero contender checks, distinct receipt integrity, and automatic reacquisition after normal and exceptional owner release. (FR-002, FR-005, FR-007, FR-008, FR-060, FR-063, FR-065, FR-068, FR-070)
- [ ] B508 Validate every successful Django module attempt creates exactly one regular contained nonempty bounded shard, normalize it to private mode, and reject or safely unlink invalid collateral before admitting success. (FR-002, FR-005, FR-007, FR-008, FR-031, FR-060, FR-063, FR-065, FR-068, FR-070)
- [ ] B509 Revalidate the exact complete private shard inventory and one-shard-per-module cardinality immediately before coverage combination. (FR-002, FR-005, FR-007, FR-008, FR-060, FR-063, FR-065, FR-068, FR-070)
...
## Phase 70 — Private evidence and lock cycle 93 corrections

- [ ] B524 Traverse and create complete-gate evidence only through contained no-follow directory descriptors beneath the trusted repository root, enforcing owned directories at mode 0700. (FR-002, FR-005, FR-008, FR-031, FR-060, FR-065, FR-068, FR-070)
- [ ] B525 Create every log and JSON receipt as an owned regular no-follow member at mode 0600, using exclusive private temporary members and atomic same-directory replacement without following a pre-existing target. (FR-002, FR-005, FR-008, FR-031, FR-060, FR-065, FR-068, FR-070)
- [ ] B526 Bind each complete-gate artifact's exact name, SHA-256, and byte size into the canonical result digest and revalidate the manifest plus every member before admitting or replaying evidence. (FR-002, FR-005, FR-008, FR-060, FR-063, FR-065, FR-068, FR-070)
- [ ] B527 Open complete-gate and isolated-E2E locks through contained private parents with `O_NOFOLLOW`, verify owned regular file identity, and retain nonblocking exit 3 plus automatic kernel release. (FR-002, FR-005, FR-007, FR-008, FR-031, FR-060, FR-065, FR-068, FR-070)
- [ ] B528 Replace the isolated-E2E fixed readiness member with an inherited anonymous pipe and place proof-only logs in a fresh private temporary directory removed on every terminal path. (FR-002, FR-005, FR-008, FR-031, FR-060, FR-065, FR-068, FR-070)
...
## Phase 72 — Inherited lock capability and cleanup cycle 95 corrections

- [ ] B535 Remove every ambient-variable lock bypass and make the isolated E2E public entrypoint unconditionally acquire the fixed private kernel lock. (FR-002, FR-005, FR-007, FR-008, FR-031, FR-060, FR-065, FR-068, FR-070)
- [ ] B536 Admit the fixed E2E body only after an inherited descriptor is verified as an owned single-link mode-0600 regular member with the exact expected device and inode and an active compatible lock. (FR-002, FR-005, FR-007, FR-008, FR-031, FR-060, FR-065, FR-068, FR-070)
- [ ] B537 Make complete-gate evidence replay read-only, rejecting rather than creating or repairing missing, linked, hardlinked, wrong-owner, wrong-mode, or nonregular directories and members. (FR-002, FR-005, FR-008, FR-031, FR-060, FR-063, FR-065, FR-068, FR-070)
- [ ] B538 Close every newly opened directory, lock, log, result, verifier, readiness, and child-launch descriptor on fstat, fchmod, flock, write, fdopen, subprocess-setup, and validation exceptions, then release every acquired kernel lock. (FR-002, FR-005, FR-007, FR-008, FR-060, FR-065, FR-068, FR-070)
