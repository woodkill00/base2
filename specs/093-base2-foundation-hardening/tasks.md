...
- [ ] T131 [P] [US1] Write hostile live-adapter tests for exact droplet identity/tag, fixed image/region/size/key, bounded waits, SSH bootstrap, single-record DNS replacement, health, compare-before-delete, redaction, and zero-resource inventory | Depends: T044,T118 | Validate: live adapter fixture matrix fails before T132 and passes without network afterward
- [ ] T132 [US1] Implement the bounded DigitalOcean preview adapter and exact-source remote bootstrap with no request-supplied command, production certificate, broad DNS, or retained-resource authority | Depends: T131 | Validate: adapter matrix and providerless lifecycle pass; unsupported input fails before mutation
- [ ] T133 [P] [US1] Add an explicit single-host Traefik canary route/config and live runner contract bound to exact plan/commit/archive/lease/cost/trial limits | Depends: T031,T131 | Validate: exact one-name SAN/route tests, staging endpoint assertion, interruption cleanup, and runner dry-run receipt
- [ ] T045 [US1] Under separate live approval, run three deploy/verify/destroy canaries and reconcile provider/DNS/cost to zero | Depends: T025,T044,T118,T132,T133 | Validate: three integrity-bound zero-resource receipts
