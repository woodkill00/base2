...
export default function ContentWorkspace() {
  ...
  const requestedTabKey = location.hash.slice(1).toLowerCase();
  const tab = TAB_HASHES[requestedTabKey] || 'Records';
  const canonicalTabHash = TAB_HASHES[requestedTabKey] ? `#${requestedTabKey}` : '#records';
  ...
  const moveTabFocus = (event, index) => {
    ...
    selectTab(nextTab);
    globalThis.document?.getElementById(`workspace-tab-${nextTab.toLowerCase()}`)?.focus();
  };
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-8">
        ...
        >
          {TABS.map((item, index) => (
            <GlassButton
              key={item}
              id={`workspace-tab-${item.toLowerCase()}`}
              role="tab"
        ...
        <div
          ...
          role="tabpanel"
          aria-labelledby={`workspace-tab-${tab.toLowerCase()}`}
        >
          {error ? (
