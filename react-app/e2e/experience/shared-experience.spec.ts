...
test('new routes require an explicit experience review', ({}, testInfo) => {
  const source = readFileSync(
    `${testInfo.config.rootDir}/../../src/routes/PublicRoutes.jsx`,
    'utf8'
  );
  const expected = [
    '/',
  ...
  expect([...source.matchAll(/path="([^"]+)"/g)].map((match) => match[1])).toEqual(expected);
});
...
test('signup explains policy, associates validation and keeps keyboard focus visible', async ({
  page,
}, testInfo) => {
  await page.route('**/api/**', (route) => {
    if (route.request().url().endsWith('/auth/register'))
      return route.fulfill({
        status: 422,
        json: { detail: [{ loc: ['body', 'password'], msg: 'Password does not meet policy' }] },
      });
    return route.fulfill({ status: 401, json: { detail: 'Not authenticated' } });
  });
  ...
  expect(
    await page.locator('#password').evaluate((el) => getComputedStyle(el).outlineStyle)
  ).not.toBe('none');
