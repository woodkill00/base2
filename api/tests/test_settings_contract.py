...
def test_avatar_url_allows_empty_or_public_https_without_fetching():
    ...
    assert (
        _safe_avatar_url('https://cdn.example.com/avatar.png')
...
def test_django_and_api_settings_schema_contracts_exist():
    ...
    django_migration = (
        root / 'django/users/migrations/0008_notificationpreference_userpreferenceset.py'
    ).read_text()
...
def _admit_settings(monkeypatch):
    ...
    monkeypatch.setattr(
        'api.routes.settings.require_authenticated_principal', lambda request: principal
    )
...
def test_preferences_endpoint_is_tenant_bound_audited_and_conflict_safe(monkeypatch):
    ...
    captured = {}
    monkeypatch.setattr(
        'api.routes.settings.repository.update_preferences',
        lambda **kwargs: captured.update(kwargs) or {**valid_preferences(), 'version': 1},
    )
    monkeypatch.setattr(
        'api.routes.settings.insert_audit_event',
...
def test_preferences_reject_invalid_locale_timezone_and_unknown_fields_before_write(monkeypatch):
    ...
    writes = []
    monkeypatch.setattr(
        'api.routes.settings.repository.update_preferences', lambda **kwargs: writes.append(kwargs)
    )
    invalid_locale = TestClient(app).put(
        '/api/settings/preferences', json=valid_preferences(locale='xx-invalid')
    )
    invalid_timezone = TestClient(app).put(
        '/api/settings/preferences', json=valid_preferences(timezone='Not/AZone')
    )
...
def test_notification_endpoint_preserves_required_families_and_audits_exact_count(monkeypatch):
    ...
    captured = {}
    monkeypatch.setattr(
        'api.routes.settings.repository.replace_notifications',
        lambda **kwargs: captured.update(kwargs) or kwargs['preferences'],
    )
    monkeypatch.setattr(
        'api.routes.settings.insert_audit_event',
        lambda **kwargs: captured.setdefault('audit', kwargs),
