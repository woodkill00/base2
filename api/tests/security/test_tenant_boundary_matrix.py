...
def test_pool_checkout_binds_exact_tenant_and_resets_on_return(monkeypatch):
    ...
    assert conn.calls == [("SELECT set_config('app.tenant_id', %s, true)", ('tenant-a',))]
...
def test_database_dsn_encodes_credentials_and_adds_verified_tls(monkeypatch):
    ...
    explicit = db._build_dsn()
    assert explicit.count('sslmode=') == 1
    assert 'sslmode=verify-full' in explicit
    assert 'application_name=base2' in explicit
