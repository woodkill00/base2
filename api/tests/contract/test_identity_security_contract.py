...
def test_google_oauth_start_uses_exact_allowlisted_redirect_and_browser_binding(monkeypatch):
    ...
    assert (
        'redirect_uri=https%3A%2F%2Fexample.test%2Fapi%2Foauth%2Fgoogle%2Fcallback'
        in body['authorization_url']
    )
...
def test_oauth_state_rejects_external_return_paths_expiry_tampering_and_wrong_browser():
    ...
    assert (
        signer.verify(issued.state, browser_binding=issued.browser_binding, now=now)['return_path']
        == '/account'
    )
...
def test_rbac_is_deny_by_default_and_sensitive_actions_need_recent_reauthentication():
    ...
    assert is_allowed('owner', 'content-workspace.publish') is True
    assert is_allowed('admin', 'content-workspace.delete') is True
    assert is_allowed('editor', 'content-workspace.schedule') is True
    assert is_allowed('editor', 'content-workspace.publish') is False
    assert is_allowed('viewer', 'content-workspace.schedule') is False
