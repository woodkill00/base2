...
def test_community_is_durable_pending_and_author_bound():
    repo = Repository()
    service = SiteContentService(repo)
    result = service.submit_community(
        site_id='tenant-one',
        author_ref='user-1',
        title='Useful title',
        body='A useful community contribution.',
    )
...
def test_support_requires_processing_consent_and_stores_private_shape():
    ...
    with pytest.raises(EngagementPolicyError, match='consent'):
        service.submit_form(
            ...
            form_key='support',
            replay_key='x',
            payload={'subject': 'Need help', 'message': 'A sufficiently long support message.'},
            consent={},
            request_id='r',
        )
    service.submit_form(
        ...
        replay_key='x',
        payload={'subject': 'Need help', 'message': 'A sufficiently long support message.'},
        consent={'supportProcessing': True},
        request_id='r',
    )
