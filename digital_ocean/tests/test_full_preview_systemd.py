...
def test_expiry_timer_calls_one_unified_lifecycle_operation():
    ...
    assert (
        "/usr/bin/flock --exclusive /var/lib/base2/full-preview/.expiry-operation.lock" in service
    )
...
def test_timer_has_no_parallel_or_legacy_dns_cleanup_command():
    combined = "\n".join(
        path.read_text()
        for path in (ROOT / "digital_ocean/systemd").glob("base2-full-preview-expiry@.*")
    )
...
def test_remote_renderer_runs_as_repository_module_on_a_fresh_host():
    ...
    assert (
        'python3 "$repo_root/digital_ocean/scripts/python/render_full_preview_env.py"' not in script
    )
...
def test_media_enabled_preview_builds_and_starts_isolated_inspector_with_image_identity():
    ...
    assert (
        '[[ "$clamav_state" == "running" && "$clamav_health" == "healthy" && "$clamav_oom" == "false" ]]'
        in script
    )
...
def test_remote_bootstrap_accepts_only_the_successful_workspace_role_one_shot():
    ...
    assert 'COMPOSE_PARALLEL_LIMIT=2' in script
