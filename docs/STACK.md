...
- api: FastAPI external API under `/api/*` (pass-through routing); canonical auth surface `/api/auth/*`
- django: canonical domain models, migrations, admin (admin/schema only; no auth API routes)
...
- Host `${WEBSITE_DOMAIN}` path `/api/*` -> api (pass-through)
